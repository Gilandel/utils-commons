...
public final class CastUtils {
	...
	/**
	 * Cast an object into a typed {@link TreeMap}. Comparator has to be null safe.
	 * 
	 * @param o
	 *            The input object
	 * @param classKey
	 ...
	 *            The type of the value
	 */
	public static <K, V> Map<K, V> getLinkedHashMap(final Object o, final Class<K> classKey,
			final Class<V> classValue) {
	 ...
	 * 
	 */
	private static <K, V> void setMapValue(final Map<K, V> map, final K key, final Entry<?, ?> obj,
			final Class<V> classValue, final boolean removeNull) {
	 ...
	 *            If null has to be removed
	 */
	private static <K, V> void map(final Map<K, V> map, final Object o, final Class<K> classKey,
			final Class<V> classValue, final boolean removeNull) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	protected static <T> T instantiate(final boolean logLevelInfo, final Constructor<?> constructor,
		...
		try {
			...
			if (logLevelInfo) {
				LOGGER.info("Cannot instantiate {} with argurments {}", constructor.getName(),
						StringUtils.joinComma(objects));
			} else {
				LOGGER.error("Cannot instantiate {} with argurments {}", constructor.getName(),
						StringUtils.joinComma(objects));
			}
		}
		return result;
	 ...
	 *            The type of the element
	 */
	protected static <T> T instantiateConstructor(final Class<T> instantiableClass, final Class<?>[] classes,
			final Object[] objects) {
		T result = null;
	 ...
	 */
	@SuppressWarnings("unchecked")
	public static <K, V> Class<Map<K, V>> getTypedMapClass(final Class<K> instantiableKeyClass,
			final Class<V> instantiableValueClass) {
		final Map<K, V> map = new HashMap<>();

		if (instantiableKeyClass != null && instantiableValueClass != null) {
			try {
				map.put(instantiableKeyClass.getDeclaredConstructor().newInstance(),
						instantiableValueClass.getDeclaredConstructor().newInstance());
