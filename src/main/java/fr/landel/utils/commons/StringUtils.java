...
public final class StringUtils extends StringFormatUtils {
	...
	public static final Pair<String, String> EXCLUDE_DOLLAR_CURLY_BRACES = Pair.of(DOLLAR_BRACE_OPEN_EXCLUDE,
			BRACE_CLOSE_EXCLUDE);
	 ...
	 * @param <C> Type of the char sequence
	...
	/**
	 * Get the string part between the separator at the index position. The index
	 * starts from the left at 0, or at -1 from the right. If the index is over the
	 * number of split parts, the method returns an empty string.
	 * 
	 * <pre>
	...
	/**
	 * Get the string part between the separator at the index position. The index
	 * starts from the left at 0, or at -1 from the right. If the from index is over
	 * the number of split parts, the method returns an empty string (the to index
	 * isn't checked).
	 * 
	 * <pre>
	 ...
	 * @param str       The input string
	 * @param separator The string separator
	 * @param from      The from index position (inclusive)
	 * @param to        The to index position (exclusive)
	 * @return The sub-string
	 */
	 ...
	 * @throws IllegalArgumentException If input string is null or empty. If
	 *                                  replacement string is null. If the start is
	 *                                  lower than 0. If the start is lower than the
	 *                                  end. If the end is greater than the string
	 *                                  length.
	 */
	...
	/**
	 * Converts the char sequence in char array. For {@link String}, you should use
	...
	/**
	 * <p>
	 * Joins the elements of the provided iterable into a single String containing
	 * the provided list of elements.
	 * </p>
	 *
	 * <p>
	 * No delimiter is added before or after the list. A {@code null} separator is
	 * the same as an empty String (""). The formatter if provided should support
	 * {@code null} value. By default the formatter used the function
	 * {@link String#valueOf}.
	 * </p>
	 *
	 * <pre>
	 ...
	 * @param formatter the formatter to stringify each element, null treated as
	 *                  {@link String#valueOf}
	...
	/**
	 * <p>
	 * Joins the elements of the provided iterator into a single String containing
	 * the provided list of elements.
	 * </p>
	 *
	 * <p>
	 ...
	 * </pre>
	 * 
	 * @param iterator  the {@link Iterator} providing the values to join together,
	 *                  may be null
	 * @param separator the separator character to use, null treated as ""
	 * @param formatter the formatter to stringify each element, null treated as
	 *                  {@link String#valueOf}
	 * @param <T>       the type of iterator element
	 * @return the joined String, {@code null} if null array input.
	 */
	public static <T> String join(final Iterator<T> iterator, final String separator,
			final Function<T, String> formatter) {
		if (iterator == null) {
			return null;
		}
	 ...
	 * @throws ArrayIndexOutOfBoundsException if<br>
	...
	/**
	 * <p>
	 * Joins the elements of the provided map into a single String containing the
	 * provided list of elements. On each map entry, the formatter is applied.
	 * </p>
	 *
	 * <p>
	 * No delimiter is added before or after the list. A {@code null} separator is
	 * the same as an empty String (""). If no formatter is specified, the format
	 * used is the following "key = value". Null key or value within the map are
	 * represented by the word "null".
	 * </p>
	 *
	 * <pre>
	 * StringUtils.join(null, ",", null) = null
	 ...
	 * </pre>
	 *
	 * @param map       the map of entries to join together, may be null
	 * @param separator the separator character to use, null treated as ""
	 * @param formatter the formatter to stringify each entry, null treated as
	 *                  {@code "key = value"}
	 * @param <K>       the type of each key
	 * @param <V>       the type of each value
	 * @return the joined String, {@code null} if null map input
	 */
	public static <K, V> String join(final Map<K, V> map, final String separator,
			final Function<Entry<K, V>, String> formatter) {
		if (map == null) {
			return null;
		}
	 ...
	 * <p>
	 * Joins the elements of the provided array into a single String containing the
	 * provided list of elements. Each element is separated by a comma followed by a
	 * space.
	 * </p>
	 *
	 * <p>
	 ...
	 * </pre>
	 *
	 * @param elements the array of values to join together, may be null
	 ...
	 *
	 * <p>
	 * No delimiter is added before or after the list. The comma followed by a space
	 ...
	 * </pre>
	 * 
	 * @param charSequence the input char sequence
	 * @param arguments    the arguments to inject
	...
	/**
	 * Injects all arguments in the specified char sequence. The arguments are
	 * injected by replacement of keys between braces. To exclude keys, just double
	 * braces (like {{key}} will return {key}). If some keys aren't found, they are
	 * ignored.
	 * 
	 * <p>
	 * precondition: {@code charSequence} cannot be {@code null}
	 ...
	 * 		Pair.of("where", "beach"), Pair.of("when", "afternoon"));
	 * // =&gt; "I'll go to the beach this afternoon"
	 * 
	 ...
	 * </pre>
	 * 
	 * @param include      the characters that surround the property key to replace
	 * @param exclude      the characters that surround the property key to exclude
	 *                     of replacement
	 * @param charSequence the input char sequence
	 * @param arguments    the pairs to inject
	 ...
	 * 
	 * StringUtils.injectKeys("I'll go to the {where} this {when}", Pair.of("where", "beach"),
	 ...
	 */
	@SafeVarargs
	public static <T extends Map.Entry<String, Object>> String injectKeys(final CharSequence charSequence,
			final T... arguments) {
	 ...
	 *                     of replacement
	 * @param charSequence the input char sequence
	 * @param arguments    the map of pairs to inject
	 * @return the result with replacements
	 */
	public static String injectKeys(final Pair<String, String> include, final Pair<String, String> exclude,
		...
		return injectKeys(include, exclude, charSequence, arguments.entrySet()
				.toArray(CastUtils.<Map.Entry<String, Object>[]>cast(new Map.Entry[arguments.size()])));
	}
	 ...
	 * // =&gt; "I'll go to the beach this afternoon"
	 * 
	 * StringUtils.injectKeys(Pair.of("${", "}"), Pair.of("${{", "}}"), "I'll go to ${where}${{where}}${where}",
	 * 		properties);
	...
	public static String injectKeys(final Pair<String, String> include, final Pair<String, String> exclude,
		...
		return injectKeys(include, exclude, charSequence, properties.entrySet()
				.toArray(CastUtils.<Map.Entry<String, Object>[]>cast(new Map.Entry[properties.size()])));
	}
	...
	/**
	 * Concatenate objects. If one object is {@code null}, it's replaced by the word
	 * 'null', otherwise calls toString method for each object.
	 * 
	 * @param objects the objects to concatenate
	 * @return the concatenated String (may be empty if objects array is empty)
	 * @throws NullPointerException if {@code objects} array is {@code null}
	 */
	 ...
	 * 
	 * @param sequence the sequence to prefix
	 * @param prefix   the prefix
	 * @return the prefixed sequence
	 * @throws NullPointerException if {@code sequence} or {@code prefix} are
	 *                              {@code null}
	 */
	 ...
	 * 
	 * @param sequence the sequence to suffix
	 * @param suffix   the suffix
	 * @return the suffixed sequence
	 * @throws NullPointerException if {@code sequence} or {@code suffix} are
	 *                              {@code null}
	 */
